...
#include "createDyMControls.H"
  ...
  U.boundaryFieldRef().set(sidesPatchID,fvPatchField<vector>::New("noSlip", mesh.boundary()[sidesPatchID], U));
  p.boundaryFieldRef().set(sidesPatchID,fvPatchField<scalar>::New("zeroGradient", mesh.boundary()[sidesPatchID], p));
  T.boundaryFieldRef().set(sidesPatchID,fvPatchField<scalar>::New("zeroGradient", mesh.boundary()[sidesPatchID], T));
  rho.boundaryFieldRef().set(sidesPatchID,fvPatchField<scalar>::New("zeroGradient", mesh.boundary()[sidesPatchID], rho));
  ...
  forAll(C, cell)
    {
      scalar gamma = 1.2;//Cp_[cell]/Cv_[cell];
      scalar x1 = C[cell].x();
      scalar x2 = C[cell].y();
