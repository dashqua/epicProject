...
Info << "\nCorrection of Initial Variables\n" << endl;
  ...
  U.boundaryFieldRef().set(topPatchID,fvPatchField<vector>::New("zeroGradient", mesh.boundary()[sidesPatchID], U));
  ...
  T.boundaryFieldRef().set(sidesPatchID,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[sidesPatchID], T));
  T.boundaryFieldRef().set(topPatchID,fvPatchField<scalar>::New("fixedValue", mesh.boundary()[sidesPatchID], T));
